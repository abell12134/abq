...
| **持仓追踪** | 四账户 fills 聚合，自首次买入日起走势 + 买卖节点 + 舆情/短线/研究覆盖；行内挂申万行业与板块预测徽章 |
| **大盘看板 + 板块预测** | 池内温度计/连板/资金/快讯（10:00 盘中叠加）；申万一级 10/20 日相对中证 500 候选 + LLM 简报；**不改订单** |
...
| 总览 | 持仓追踪 | 大盘看板 · 板块预测 |
|:---:|:---:|:---:|
| 四线净值/累计收益/超额对比 | 147 只标的走势、行业预测与 Agent 覆盖 | 池内温度计、连板梯队；申万一级 10/20 日候选 |

| 舆情跟踪 | 操作清单 | 短线猎手 |
|:---:|:---:|:---:|
| 多源舆情 + LLM 摘要 + 近 90 日走势 | 次日调仓指令与目标持仓 | 10 日 +10% 预测、评测报告、活跃跟踪 |

![总览 — 四线累计收益与超额对比](img/pic1.png)

![持仓追踪 — 行业预测徽章、资金流向与 Agent 覆盖](img/pic4.png)

![大盘看板 · 全景 — 池内温度计与涨跌榜](img/pic5.png)

![板块预测 — 申万一级 10/20 日相对中证 500 候选 + LLM 简报](img/pic6.png)

![舆情跟踪 — 持仓/订单标的持续分析](img/pic2.png)

![操作清单 — 次日调仓指令与目标持仓](img/pic3.png)

![短线猎手 — 10 日 +10% 预测与活跃跟踪](img/pic7.png)

> 截图由 `quant/ops/capture_webapp_screenshots.py` 在本地看板 `:8000` 运行时生成（需 Playwright）。
...
1. **给仓库 Star** — 最好的免费鼓励，也让更多人发现这个项目  
   👉 **[github.com/abell12134/abq](https://github.com/abell12134/abq)** 点右上角 ⭐  
2. **微信赞赏（可选）** — 用于补贴 API 与服务器开销；金额随意，感谢认可  

<p align="center">
  <img src="img/payment.jpg" alt="微信赞赏码" width="280" />
  <br />
  <sub>扫码微信赞赏 · 自愿支持 · 与研究用途无关</sub>
</p>

> 赞赏纯属自愿，**不换取**投资建议、代操、信号订阅或任何承诺收益；本仓库始终是研究 / 学习项目。
...
- **[docs/SWING_HUNTER.md](docs/SWING_HUNTER.md)** — 短线猎手：门槛档位、数据布局、CLI、看板 API  
