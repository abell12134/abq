...
```
  ...
  └──────────────────────────────────────────┘
         │  并行：工作日 10:00 (自动)
         │  大盘看板 board_morning：池内批量报价 → intraday/latest.json
         ▼
  收盘后 (人工)
  ┌──────────────────────────────────────────┐
...
```
  源                    处理                         落盘
  ─────                 ────                         ────
  东财 JSONP 个股新闻（近 90 日）─┐
  东财/巨潮 公司公告·财报 ───────┼─→ 去重入库 → hashing 向量 ─→ raw/ + vectors/
  财联社/新浪/东财要闻·政策筛 ─┘         │
                                 ▼
