...
- **主区**：申万一级行业 10 日 / 20 日相对中证 500 占优/落后候选（`overlays/sector_forecast`）。
  每卡：行业、动作、p(跑赢)、期望超额、三条证据、相关概念；OOS 未过门则标 shadow。
  **这是预测**；结算见该 overlay 口径（T+1 起算，超额>0 为 hit）。
- **次区（证据，不是预测）**：题材热度榜、东财行业榜、池内轮动热力矩阵。
- 概念与东财行业榜不作为 claim 对象，不参与结算。

![大盘看板 · 全景 — 池内温度计与涨跌榜](../img/pic5.png)

![板块预测 — 两周/一个月候选 + LLM 简报](../img/pic6.png)
...
### 板块预测（2026-09-08，sector_forecast）

- 新 overlay `quant/overlays/sector_forecast/`：截面 LightGBM 预测池内申万一级未来 10/20 个交易日相对中证 500 超额。
- evening 在 `run_board` 之后 fail-open 接入；盘中 10:00 **不重算**（收盘口径）。
- OOS 对照「当日涨幅榜 Top3」；未显著优于基线则标 shadow，禁止把今日热度包装成预测。
- API：`GET /api/sector/forecast`、`GET /api/sector/eval`、`POST /api/sector/run`（`brief_only` 只补 LLM）。
- 选出后 LLM 写理由/风险/`agree|challenge`，不改 p_beat 与排名；失败 fail-open。
- 持仓追踪行挂 `industry` + `sector_forecast` 切片。
