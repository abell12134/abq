...
- [~] 阶段 4：路线一（人工下单）每日闭环工具链已落地（编排/回填/对账/净值/日报/监控），进入连续实跑累计验收期
- [~] 阶段 5a：后台常驻看板 + 内置定时（FastAPI:8000 + APScheduler），双线试运行攒数据中
- [ ] 阶段 5：全自动实盘（miniQMT）
...
- 配置：`research/workflow_baseline.yaml`（Alpha158 + LGBM，csi500，TopK50 / n_drop3 / hold_thresh10）
...
ops/             运维层：编排(run_daily) + 净值(compute_nav) + 日报(daily_report) + 监控(monitor) + 双线复盘(review_accounts) + 回填(backfill) + 公共库(common) + crontab.example
contracts/       层间数据契约 schema 校验/读写(schemas.py)
webapp/          （阶段5a）看板服务 server.py(FastAPI+APScheduler) + serve.sh + templates/ + static/
data/            运行时数据（signals/reports/nav/fills、accounts/<account>/ 双线隔离，不入 git）
```
...
- 五道准入关卡（§3.3 防过拟合生命线，`run_iteration.py:gates`）：
  ...
  5. **纸面跟踪**：对"现有因子库等权组合"有样本外 Rank IC 增量者，才进入 `paper_tracking`，
     先跟踪 1~3 个月、线上 IC 达标后再并入研究层特征集（滚动重训验证）
...
- 产出：`data/reports/factor_iter_*.md`（逐轮准入漏斗 + 入库因子明细 + 验收对照）
...
data/nav/account.json                start_capital,start_date,cash,last_fill_date（现金账本）
```
...
- 研究模拟线：`research_sim_100k`，启动资金 100000 元，系统按**次日开盘价**自动模拟成交，
  记录每日模拟操作与收盘净值。
- 实盘线：`live_manual_10k`，启动资金 10000 元，目标持仓 5-10 只；系统生成订单，
  用户只回填实际 `fills`，系统收盘后自动对账/净值/复盘。
...
- **compute_nav**：现金 + 持仓盯市 → 净值/超额/换手，单日亏损触及 -3% 熔断告警
...
- **monitor**：数据新鲜度、信号/成交/净值是否就绪、熔断等健康检查，告警写 `data/logs/alerts.log`
...
**试运行 → 正式实盘的切换**：试运行期 `live_manual_10k` 用 `mode: simulated`（与研究线一样自动按
次日开盘价模拟），攒满约 1 周数据验证看板与流程。正式开始真实交易时：

```bash
# 1) configs/accounts/live_manual_10k.yaml 把 mode 改回 manual
# 2) 用真实开始日重置该账户现金账本（清掉试运行模拟数据再 init）
rm -rf data/accounts/live_manual_10k
python execution/record_fills.py --account live_manual_10k --init-capital 10000 --day <真实开始日>
# 之后每个交易日：照 orders/ 清单人工下单 → 收盘后 record_fills --template→改实际成交→--apply
```
